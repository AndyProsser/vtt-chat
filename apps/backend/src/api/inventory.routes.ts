...
export async function getActiveSession(campaignId: UUID) {
  try {
    ...
    return (
      sessions.find((s) => s.state === SessionState.ACTIVE || s.state === SessionState.PAUSED) ??
      null
    )
  } catch {
    return null
...
router.get('/:campaignId/history', requireAuth, async (req: Request, res: Response) => {
  ...
  if (ownerType && !['party', 'character'].includes(ownerType)) {
    return res
      .status(400)
      .json({ code: 'INVALID_INPUT', message: 'ownerType must be party or character' })
  }
    ...
    if (role === 'DM') {
      ...
      if (ownerType === 'character') {
        if (ownerId && ownerId !== user.userId) {
          return res
            .status(403)
            .json({ code: 'FORBIDDEN', message: "Cannot view other players' inventory history" })
        }
...
router.post('/:campaignId/items', requireAuth, async (req: Request, res: Response) => {
  ...
  if (ownerType === 'character' && !isValidUUID(ownerId)) {
    return res
      .status(400)
      .json({ code: 'INVALID_INPUT', message: 'ownerId required for character items' })
  }
  ...
  if (role === 'SPECTATOR') {
    return res
      .status(403)
      .json({ code: 'FORBIDDEN', message: 'Spectators cannot modify inventory' })
  }
...
router.patch(
  ...
  async (req: Request, res: Response) => {
    ...
    if (containerId !== null && !isValidUUID(containerId)) {
      return res
        .status(400)
        .json({ code: 'INVALID_INPUT', message: 'containerId must be a valid UUID or null' })
    }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!['party', 'character'].includes(toOwnerType)) {
      return res
        .status(400)
        .json({ code: 'INVALID_INPUT', message: 'toOwnerType must be party or character' })
    }
...
router.post('/:campaignId/transfer/currency', requireAuth, async (req: Request, res: Response) => {
  const { campaignId } = req.params
  const user = (req as any).user
  ...
  if (!['party', 'character'].includes(fromOwnerType)) {
    return res
      .status(400)
      .json({ code: 'INVALID_INPUT', message: 'fromOwnerType must be party or character' })
  }
  if (!['party', 'character'].includes(toOwnerType)) {
    return res
      .status(400)
  ...
  for (const denom of DENOMS) {
    ...
    if (v < 0 || !Number.isFinite(v)) {
      return res
        .status(400)
        .json({ code: 'INVALID_INPUT', message: `${denom} must be a non-negative number` })
    }
  ...
  if (Object.keys(normalizedAmounts).length === 0) {
    return res.status(400).json({
      code: 'INVALID_INPUT',
      message: 'amounts must include at least one positive denomination',
    })
  }
  ...
  if (role === 'PLAYER') {
    ...
    if (!fromSelf && !fromPartyToSelf) {
      return res.status(403).json({
        code: 'FORBIDDEN',
        message: 'Players can only transfer from their own wallet or take from party',
      })
    }
  }
  ...
  try {
    ...
    return res.json(
      'fromWallet' in result ? { fromWallet: result.fromWallet, toWallet: result.toWallet } : result
    )
  } catch (err) {
