...
// ─── POST /api/inventory/:campaignId/loot-split/:splitId/accept ──────────────
// Player accepts their share of an active loot split within the 60s window.

import {
  acceptLootSplit,
  getSplit,
} from '@/services/inventory/loot-split.service'

router.post(
  '/:campaignId/loot-split/:splitId/accept',
  requireAuth,
  async (req: Request, res: Response) => {
    const { campaignId, splitId } = req.params
    const user = (req as any).user

    if (!isValidUUID(campaignId) || !isValidUUID(splitId)) {
      return res.status(400).json({ code: 'INVALID_INPUT', message: 'Invalid campaignId or splitId' })
    }

    const role = await resolveCampaignRole(campaignId as UUID, user.userId as UUID)
    if (!role || role === 'DM' || role === 'SPECTATOR') {
      return res.status(403).json({ code: 'FORBIDDEN', message: 'Only players can accept a loot split.' })
    }

    try {
      const result = await acceptLootSplit({
        campaignId: campaignId as UUID,
        splitId: splitId as UUID,
        userId: user.userId as UUID,
      })

      if (!result.ok) {
        const status = result.code === 'NOT_FOUND' || result.code === 'EXPIRED' ? 404 : 409
        return res.status(status).json({ code: result.code, message: result.error })
      }

      const { split, transferredItem } = result
      const wsManager: WebSocketManager | undefined = req.app.locals.wsManager

      // Broadcast item transfer to all campaign members
      if (wsManager) {
        const session = await getActiveSession(campaignId as UUID)
        const sessionId = (session?.id ?? NO_SESSION_ID) as UUID
        ...
        await wsManager.broadcastToCampaignMembers(campaignId as UUID, {
          ...
          sessionId,
          roomId: null,
          timestamp: transferredItem.updatedAt,
          payload: {
            campaignId,
            itemId: transferredItem.id,
            name: transferredItem.name,
            quantity: split.shares.find((s) => s.userId === user.userId)?.quantity ?? 1,
            fromOwnerType: 'party',
            fromOwnerId: null,
            toOwnerType: 'character',
            toOwnerId: user.userId,
            transferredByUserId: user.userId,
            transferredAt: transferredItem.updatedAt,
          },
        })

        // Broadcast LOOT_SPLIT_ACCEPTED so all clients can update the card UI
        await wsManager.broadcastToCampaignMembers(campaignId as UUID, {
          id: crypto.randomUUID() as UUID,
          type: 'INVENTORY:LOOT_SPLIT_ACCEPTED' as const,
          version: 1,
          userId: user.userId as UUID,
          userRole: role as any,
          sessionId,
      ...
      return res.json({ item: transferredItem, split })
    } catch (err) {
      logger.error('inventory.routes', 'Failed to accept loot split', { campaignId, splitId, err })
      return res.status(500).json({ code: 'INTERNAL_ERROR', message: 'Failed to accept loot split' })
    }
  }
)
