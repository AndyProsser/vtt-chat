...
compose_file="${COMPOSE_FILE:-$INSTALL_DIR/infra/docker-compose.dev.yml}"
output_path="${1:-./tmp/caddy-local-root.crt}"
output_dir=$(dirname "$output_path")

mkdir -p "$output_dir"

container_id=$(docker compose --project-directory "$INSTALL_DIR" -f "$compose_file" ps -q caddy)
...
docker compose --project-directory "$INSTALL_DIR" -f "$compose_file" exec -T caddy sh -c 'cat /data/caddy/pki/authorities/local/root.crt' > "$output_path"
