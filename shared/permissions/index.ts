...
export const PERMISSION_MATRIX: PermissionRule[] = [
  ...
  },
  {
    ...
    allowedRoles: ['DM', 'PLAYER'],
    description:
      'Send private whisper to another user (DM-visible, recipient-visible, sender-visible)',
  },
  {
    domain: 'CHAT',
    action: 'SEND_SYSTEM',
 ...
 */
export function getDomainRules(domain: string): PermissionRule[] {
  return PERMISSION_MATRIX.filter((rule) => rule.domain === domain)
}
