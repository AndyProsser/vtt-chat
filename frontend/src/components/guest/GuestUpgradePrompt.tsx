...
export function GuestUpgradePrompt({
  ...
  const submitUpgrade: SubmitEventHandler<HTMLFormElement> = async (event) => {
    ...
    <section className="mx-auto mt-4 w-full max-w-6xl rounded-ui-md border border-amber-300 bg-amber-50 p-4 text-amber-950">
      ...
      >
        <Form.Field className="block text-sm" name="email">
          <Form.Label className="mb-1 block font-medium">Email</Form.Label>
        ...
        <Form.Field className="block text-sm" name="password">
          <Form.Label className="mb-1 block font-medium">New Password</Form.Label>
          <Form.Control asChild>
            <input
              type="password"
              value={password}
