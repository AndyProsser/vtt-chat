...
describe('store system wiring', () => {
  it('exposes expected top-level slices and actions', () => {
    ...
    expect(state).toHaveProperty('toolbarCenterPaneView')
    expect(state).toHaveProperty('toolbarRightRailOpen')
    ...
    expect(typeof state.setToolbarCenterPaneView).toBe('function')
    expect(typeof state.toggleToolbarRightRail).toBe('function')
    expect(typeof state.resetToolbarActionsState).toBe('function')
  })
})
