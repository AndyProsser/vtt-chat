...
export async function getAdminCampaignExportPayload(params: {
  ...
  const exported = await buildCampaignExport(prisma, campaign.id, params.actor.userId)
  if (!exported) {
    return {
      status: 404,
      body: { error: 'Campaign not found', code: 'NOT_FOUND' },
...
export async function getAdminCampaignRecordingsPayload(params: {
  ...
  return {
    ...
    body: {
      campaign,
      recordings: recordings.map(mapRecordingForResponse),
    },
  }
}
...
export async function moveAdminCampaignPlayerPayload(params: {
  ...
  if (!room || room.sessionId !== session.id) {
    return {
      status: 404,
      body: { error: 'Room not found', code: 'NOT_FOUND' },
    }
  }
  ...
  if (!sessionMember) {
    return {
      status: 404,
      body: { error: 'Target user not in session', code: 'NOT_FOUND' },
    }
  }
