...
- **Group persistence**: Campaign `GROUP`-type rooms now survive session boundaries. When a new session starts for the same campaign, non-private groups and their environments are carried forward from the previous session (`restoreCampaignRoomsForSession`).
