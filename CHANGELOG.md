...
- LiveKit audio publish now waits for a fully connected room before attempting local track publish, deduplicates concurrent publish requests, and avoids re-publishing when a local audio publication already exists. This hardens START/PAUSE/RESUME/COOLDOWN/ENDED transitions and suppresses repeated offer/trickle errors caused by publish attempts during reconnect windows.
