...
- Frontend `SessionInit` decomposition continued (2026-05-22):
  - Extracted `frontend/src/components/session/sessionInit.types.ts` for local API/context types.
  - Extracted `frontend/src/components/session/sessionInit.utils.ts` for pure session/lobby helpers.
  - Extracted `frontend/src/components/session/sessionInit.settings.ts` for campaign/character settings normalization helpers.
  - `frontend/src/components/session/SessionInit.tsx` now consumes the co-located modules instead of owning those helper blocks inline.
...
- Frontend layout normalization pass (2026-05-22):
  ...
  - Centralized SessionInit orchestration hooks under `frontend/src/hooks/session/*`.
...
- Frontend route/workspace component map normalization (2026-05-22):
  - Renamed `SessionInit` orchestration shell to `AppInit` (`frontend/src/components/app/AppInit.tsx`) and updated route wiring.
...
- W2 release-gate enforcement is now automated in CI:
  ...
  - Added frontend reconnect-hydration integration assertions in `frontend/tests/components/SessionInit.integration.test.tsx` to verify recovered audio state is applied end-to-end (environment map, DM overrides, broadcast state).
