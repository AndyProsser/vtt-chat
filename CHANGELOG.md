...
- **Session-state comparisons now use the shared `SessionState` enum everywhere**: ~60 raw string comparisons (`state === 'ACTIVE'` etc.) across frontend slices/components, admin, and backend routes/services/repositories were converted to `SessionState.X` members, per the "never use raw strings" rule. Locally re-declared literal unions were replaced with the canonical types: `CampaignSettingsPayload.latestSessionState` (which had drifted — it was missing `CLEANUP`) and `CampaignSummary.displayState` now use `SessionState`/`CampaignDisplayState` from `@shared`; `PartyPresenceMemberSnapshot.runtimePresenceState`/`role` now use `PresenceState`/`Role`. Comparisons against genuine UI display-state unions (`CampaignDisplayState`, `InviteCampaignDisplayState`, `CampaignVisualState`) intentionally remain literal.

### Fixed — Tooling

- **`apps/queues` test scripts no longer fail the monorepo test chain**: the queues app has no test files yet, and `vitest run` exits 1 on "No test files found", so root `npm test` could never pass end-to-end. Added `--passWithNoTests` to its `test`/`test:ci`/`test:coverage` scripts.
