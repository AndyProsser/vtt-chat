...
- **`MicLevelMeter` render loop converted from rAF to a ~30Hz timer**: the meter leaf inside `AudioDevicePanel` (rendered unconditionally, so always mounted in a session) ran a per-frame `requestAnimationFrame` loop for the whole session even when the level was a flat zero, pinning the refresh driver. It now writes the CSS variable from a `setInterval`, letting the refresh driver sleep when nothing animates.
