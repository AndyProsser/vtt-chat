...
- ESLint toolchain migrated to v10 at repo root (flat config only), legacy `.eslintrc*` files were removed, and React lint rules now run through `@eslint/compat` as a temporary bridge until `eslint-plugin-react` publishes native ESLint 10 peer support.
- Added root `.npmrc` with `legacy-peer-deps=true` so fresh installs in WSL and CI-like environments remain reproducible while the `eslint-plugin-react` peer range lags ESLint 10.
- Added package-local ESLint flat-config entrypoints (`backend/eslint.config.cjs`, `frontend/eslint.config.cjs`, `admin/eslint.config.cjs`) so ESLint 10 monorepo lookup resolves the nearest config when linting from package folders.
- Updated developer setup docs with explicit WSL-first lint/install commands and the temporary ESLint 10 peer-compatibility policy.
...
- Added a local CI-style lint gate command (`npm run ci:lint:linux`) backed by `scripts/qa/lint-linux-gate.cjs`; it runs `lint:packages` directly on Linux/macOS and via WSL bash on Windows so lint enforcement always follows Linux semantics.
...
- Added `scripts/qa/check-eslint-react-peer.cjs` plus npm commands (`qa:eslint-react-peer-check`, `ci:lint:strict`) to automatically detect when `eslint-plugin-react` publishes ESLint 10 peer support and flag whether the `.npmrc` `legacy-peer-deps=true` workaround should be kept or removed.
