...
| Stage 8 (Admin and Ops) security closure    | In progress (role hierarchy in place, invite/moderation flows pending) | Telemetry endpoints are implemented and mounted: [backend/src/api/admin.routes.ts](backend/src/api/admin.routes.ts), [backend/src/api/index.ts](backend/src/api/index.ts). Unified user-admin model is now active with admin role claims (`SUPER_ADMIN`, `ADMIN`, `CAMPAIGN_DM`, `READ_ONLY`) via [backend/prisma/schema.prisma](backend/prisma/schema.prisma), [backend/src/services/admin.service.ts](backend/src/services/admin.service.ts), and [backend/src/utils/auth.ts](backend/src/utils/auth.ts). Admin middleware is enforced for telemetry routes and role-grant endpoint (`/api/admin/users/:userId/promote`) is available. Current gap: invite-link onboarding, moderation action workflows, and deeper audit UX remain pending.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
