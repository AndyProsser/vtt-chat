...
To generate **spectator invite links** and access the admin console, the DM must hold a full vtt-chat account. Guest DMs operating via the extension can launch and manage sessions but cannot create spectator invites or access admin until they upgrade.

Guest DMs may see an `Open Admin` entry point in the main frontend for discoverability, but admin access is blocked until account upgrade. Clicking launch must present an upgrade flow and must not issue an admin token.
...
A full account is required to generate **spectator invite links** and to access the admin console. Guest DMs can launch sessions, manage players, and control session state, but cannot create spectator invites or access admin until they upgrade to a full account.
